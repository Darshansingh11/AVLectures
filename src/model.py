...
class Net(nn.Module):
    def __init__(
        ...
        #test:
        # self.GU_vid1 = Gated_Embedding_Unit(
        #     4096, ocr_dim, gating=True
        # )
        # self.GU_vid2 = Gated_Embedding_Unit(
        #     4096, embd_dim, gating=True
        # )
    ...
    # def forward(self, video, text, ocr):
    #     if ocr != None:
    #         ocr_e = self.GU_ocr(self.ocr_pooling(ocr))
    #     video = self.GU_vid1(video)
    #     if ocr != None:
    #         video = th.cat((video, ocr_e), dim=1)
    #         video = self.GU_vid2(video)
    #     text = self.GU_text(self.text_pooling(text))
