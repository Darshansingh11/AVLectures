...
Path(join(base_dir, "features_m011", "2d")).mkdir(parents=True, exist_ok=True)
Path(join(base_dir, "features_m011", "3d")).mkdir(parents=True, exist_ok=True)

fields = ['video_path', 'feature_path']

filename_2d = join(base_dir, 'input_2d_m011.csv') # for extracting 2d features
filename_3d = join(base_dir, 'input_3d_m011.csv') # for extracting 3d features
	...
	for folder in folder_list:
		...
		with open(join(folder, 'combined.txt'), 'r') as text_file:
			...
			for line in lines:
				...
				vid_path = join(folder, 'splits_vid', vid_name)
				feature_name = vid_name.replace('.mp4', '.npy')
				feature_path_2d = join(base_dir, 'features_m011', '2d', feature_name)
				feature_path_3d = join(base_dir, 'features_m011', '3d', feature_name)
