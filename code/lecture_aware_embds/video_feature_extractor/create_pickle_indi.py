...
for fl in glob(join(base_dir, '*')):
	...
	if ('mit006' in fl) or ('mit011' in fl):
		folder_list.append(fl)
...
for folder in folder_list:
	...
	with open(join(folder, 'combined.txt'), 'r') as text_file:
		...
		for line in lines:
			...
			if isfile(join(base_dir, 'features_m011', '2d', features_name)) and  isfile(join(base_dir, 'features_m011', '3d', features_name)):

				two_d = np.load(join(base_dir, 'features_m011', '2d', features_name))
				three_d = np.load(join(base_dir, 'features_m011', '3d', features_name))
			...
			elif isfile(join(base_dir, 'features', '2d', features_name)) and  isfile(join(base_dir, 'features', '3d', features_name)):
...
with open(join(base_dir,  'm6m11_pysd.pkl'), 'wb') as handle:
