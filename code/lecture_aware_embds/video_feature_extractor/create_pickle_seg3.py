...
for folder in folder_list:
    ...
    with open(join(folder, 'combined.txt'), 'r') as text_file:
        ...
        for line in tqdm(lines):
            ...
            features_name = "-".join(vid_name.split('-')[:-1])
            features_name = (features_name + '.npy')
            
            # if course_name == 'mit001':
            #     print(features_name, st, et)
            ...
            if isfile(join(base_dir, 'features', '2d', features_name)) and isfile(join(base_dir, 'features', '3d', features_name)):
                ...
                two_d_full = np.load(join(base_dir, 'features', '2d', features_name)) 
                three_d_full = np.load(join(base_dir, 'features', '3d', features_name))

                from math import ceil
                two_d = two_d_full[ceil(st):ceil(et)]
                three_d = three_d_full[(ceil(st * 1.5)):(ceil(et * 1.5))] 
