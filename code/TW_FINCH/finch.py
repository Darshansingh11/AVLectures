...
def clust_rank(mat, initial_rank=None, distance='cosine', use_tw_finch=False, alpha=1):
    s = mat.shape[0]
    ...
    if initial_rank is not None:
        ...
        orig_dist = orig_dist * (loc_dist ** alpha)
...
def req_numclust(c, data, req_clust, distance, use_tw_finch=False, alpha = 1):
    iter_ = len(np.unique(c)) - req_clust
    c_, mat = get_merge([], c, data)
    for i in range(iter_):
        adj, orig_dist = clust_rank(mat, initial_rank=None, distance=distance, use_tw_finch=use_tw_finch, alpha=alpha)
...
def FINCH(data, initial_rank=None, req_clust=None, distance='cosine', tw_finch=True, ensure_early_exit=False, verbose=True, alpha = 1):
    ...
    min_sim = None
    adj, orig_dist = clust_rank(data, initial_rank, distance=distance, use_tw_finch=tw_finch, alpha = alpha)
    initial_rank = None
    ...
    while exit_clust > 1:
        adj, orig_dist = clust_rank(mat, initial_rank, distance=distance, use_tw_finch=tw_finch, alpha = alpha)
    ...
    if req_clust is not None:
        if req_clust not in num_clust:
            ...
            req_c = req_numclust(c[:, ind[-1]], data, req_clust, distance, use_tw_finch=tw_finch, alpha = alpha)
        else:
...
def main():
    ...
    for idx, i in enumerate(sorted(os.listdir(dr))):

        if idx == 0:
            a = np.load(os.path.join(dr, i))
        else:
            b = np.load(os.path.join(dr, i))
            a = np.concatenate((a, b))
    data = a
    start = time.time()
    c, num_clust, req_c = FINCH(data, initial_rank=None, req_clust=5, distance='cosine', ensure_early_exit=True, verbose=True)
